...
export const createAccount = async (req, res) => {
  ...
  try {
    ...
    const existingUser = await AccountRepository.findOne({ EmailAddress: req.body.email });
    if (existingUser) {
   ...
   const newAccount = {
    ...
    leadPastorName:req.body.leadPastorName,
    ChurchAdminName:req.body.ChurchAdminName,
    ChurchHQAddress:req.body.ChurchHQAddress,
    ChurchContactPhoneNumber:req.body.ChurchContactPhoneNumber
   }

  
      const account = await AccountRepository.create(newAccount);
      //await WelcomeEmail(newUser.email, newUser.fullName)
      res.status(201).send({account, message:"account created successfully" });
